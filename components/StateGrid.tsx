    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
      {sortedStates.map(([code, name]) => {
        ...
        return (
          ...
          >
            <span className="font-medium text-gray-700 dark:text-gray-300 group-hover:text-treasure-800 dark:group-hover:text-treasure-300 text-sm">
              {name}
            </span>
            {count > 0 && (
              <span className="text-xs bg-treasure-100 dark:bg-treasure-900/30 text-treasure-700 dark:text-treasure-300 px-2 py-0.5 rounded-full">
                {count}
              </span>
            )}
          </Link>
